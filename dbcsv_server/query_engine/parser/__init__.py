...
class Parser:
        ...
        try:
            ...
            if regex_end_character.search(self.current_character):
                ...
                return self.peek_next(
                    whole_literal + self.current_character, regex_end_character
                )
    ...
    def scan_number(self) -> str:
        ...
        scan_number = scan_util(
            self.current_character, re.compile(r"[ \t\r\n\,\*\/\+\-\.\<\>\=\(\)]")
        )
        if self.current_character == ".":
            # meet dot then scan again
            scan_number = scan_util(
                scan_number + self.current_character,
                re.compile(r"[ \t\r\n\,\*\/\+\-\.\<\>\=\(\)]"),
            )
            return scan_number
        return scan_number
        ...
        while keep_scan:
            try:
                match self.current_character:
                    ...
                    case "'":  # String literal
                        self.token.append(
                            (Token.STRING_LITERAL, self.peek_next("", re.compile(r"'")))
                        )
                    case _:  # Reversed word | Column name | Table name | Not support
                        ...
                        # Ignore character space|\t|\r\n
                            ...
                            raise ValueError(
                                f"Not support character {self.current_character} at line {self.current_line} position {self.current_position}"
                            )
                self.next_character()
            except StopIteration as e:
                keep_scan = False
    ...
    def parse(self) -> AST:
        ...
        if self.match_token(ReservedWord.SELECT):
            ...
            raise ValueError(
                f"Invalid query expect SELECT or CREATE TABLE command got f{self.current_token}"
            )
        return ast
    ...
    def parse_select_clause(self) -> SelectNode:
        ...
        return SelectNode(
            column_list=column_list, from_clause=from_clause, where_clause=where_clause
        )
    ...
    def parse_column_list(self) -> ColumnListNode:
        ...
        """
        #
        current_left = ColumnListNode(
            left=self.parse_column(), right=None, operator=None
        )
        while self.match_token(Token.COMMA):
            ...
            current_left = ColumnListNode(
                left=previous_left, right=self.parse_column(), operator=operator
            )
        return current_left
    ...
    def parse_column(self) -> ColumnNode:
        ...
        if self.match_token(Token.ASTERISK):
            ...
            column_node.name = " ".join(
                [
                    str(value[1]) if len(value) > 1 else value[0].value
                    for value in self.token[star_index - 1 : self.token_index - 1]
                ]
            )
            ...
            if self.current_token and not self.match_token(ReservedWord.FROM):
                if self.match_token(Token.COMMA):
                    return column_node
                elif self.match_token(ReservedWord.AS) or self.match_token(
                    Token.IDENTIFIER
                ):
                    self.advance_token()
