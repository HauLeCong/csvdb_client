...
from ..ast_node import ExprNode, ExprAddNode, ExprMultiNode, ExprValueNode, ExprParentNode, ValueNode
from . import Parser
...
class ExpressionParser:
    ...
    def _parse_expr_add(self) -> ExprAddNode:
        ...
        while self._caller.current_token and (self._caller.match_token(Token.PLUS) or self._caller.match_token(Token.MINUS)):
            current_operator = self._caller.current_token[0]
            self._caller.advance_token()
            previous_left = current_left
            current_left = ExprAddNode(left=previous_left, right=self._parse_expr_multi(), operator = current_operator)
        return current_left
    ...
    def _parse_expr_multi(self) -> ExprMultiNode:
        ...
        while self._caller.current_token and (self._caller.match_token(Token.ASTERISK) or self._caller.match_token(Token.DIVIDE)):
            ...
            previous_left = current_left
            self._caller.advance_token()
            current_left = ExprMultiNode(left= previous_left, right=self._parse_expr_value(), operator=current_operator)
        return current_left
    ...
    def _parse_expr_value(self) -> ExprValueNode:
        ...
        Returns:
            `ExprValueNode`
        """
        if self._caller.current_token and self._caller.match_token(Token.NUMBER_LITERAL) or self._caller.match_token(Token.STRING_LITERAL):
