...
class PredicateParser:
    ...
    def _parse_predicate_or_node(self) -> PredicateOrNode:
        ...
        while self._caller.current_token and self._caller.match_token(ReservedWord.OR):
            ...
            current_left = PredicateOrNode(left=previous_left, right=self._parse_predicate_and_node(), operator=operator)
        return current_left
    ...
    def _parse_predicate_and_node(self) -> PredicateAndNode:
        ...
        while self._caller.current_token and self._caller.match_token(ReservedWord.AND):
            ...
            operator = self._caller.current_token[0]
            self._caller.advance_token()
            current_left = PredicateAndNode(left=previous_left, right=self._parse_predicate_not_node(), operator=operator)
        return current_left
    ...
    def _parse_predicate_not_node(self) -> PredicateNotNode:
        if self._caller.match_token(ReservedWord.NOT):
            operator = self._caller.current_token[0]
            self._caller.advance_token()
            return PredicateNotNode(expr=self._parse_predicate_compare_node(), operator=operator)
        return PredicateNotNode(expr=self._parse_predicate_compare_node(), operator=None)
    
    def _parse_predicate_compare_node(self) -> PredicateCompareNode:
        if self._caller.match_token(Token.LEFT_PAREN):
            self._caller.advance_token()
            predicate_parent = PredicateCompareNode(left=self._parse_predicate_parent(), right=None, operator=None)
            if not self._caller.match_token(Token.RIGHT_PAREN):
                raise ValueError(f"Expected close ) got {self._caller.current_token}")
            return predicate_parent
        else:
            current_left = PredicateCompareNode(left=self._caller.parse_expr(), right= None, operator=None)
            while self._caller.current_token\
                and (self._caller.match_token(Token.EQUAL)\
                ...
                self._caller.advance_token()
                previous_left = current_left
                current_left = PredicateCompareNode(left=previous_left, right=self._caller.parse_expr(), operator=current_operator)
            return current_left
        
    def _parse_predicate_parent(self) -> PredicateParentNode:
        predicate_node = PredicateParentNode(expr=self.parse())
